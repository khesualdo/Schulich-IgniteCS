...
void keyPressed(){
  ...
  // Move the shield to the left, but
  // never off the screen
  if (keyCode == LEFT && xRect > 0){
    xRect -= speedRect;
  }
  
  // Move the shield to the right, but
  // never off the screen
  if (keyCode == RIGHT && (xRect + widthRect) < width){
    xRect += speedRect;
  }
}
